...
@app.post("/auth/login", tags=['User Service'])
async def login(user_data: UserCredentials):
    try:
        response = requests.post(f"{USER_SERVICE_URL}/api/token", json={"username": user_data.username, "password": user_data.password})
...
@app.post("/auth/register", tags=['User Service'])
async def registeration(user_data:UserRegisteration):
    try:
        response = requests.post(f"{USER_SERVICE_URL}/api/users", json={"name":user_data.name,"email": user_data.email, "password": user_data.password})
        if response.status_code == 200:
            return response.json()
        else:
            raise HTTPException(status_code=response.status_code, detail=response.json())
...
@app.post("/auth/generate_otp", tags=['User Service'])
async def generate_otp(user_data:GenerateOtp):
    try:
        response = requests.post(f"{USER_SERVICE_URL}/api/users/generate_otp", json={"email":user_data.email})
        if response.status_code == 200:
            return response.json()
        else:
            raise HTTPException(status_code=response.status_code, detail=response.json())
...
@app.post("/auth/verify_otp", tags=['User Service'])
async def verify_otp(user_data:VerifyOtp):
    try:
        response = requests.post(f"{USER_SERVICE_URL}/api/users/verify_otp", json={"email":user_data.email ,"otp":user_data.otp})
        if response.status_code == 200:
            return response.json()
        else:
            raise HTTPException(status_code=response.status_code, detail=response.json())
